...
class GetUserDataUseCaseShould {
    ...
    @Before
    fun setUp() {
        getUserDataUseCase = GetUserDataUseCaseImpl(userDataRepository)
    }
    ...
    fun `Get UserData when getUserData is success`() = runTest {
        ...
        whenever(userDataRepository.getUserData(ANY_USER_ID)).thenReturn(flowOf(resultSuccess))

        val result = getUserDataUseCase(ANY_USER_ID).firstOrNull()

        verify(userDataRepository).getUserData(ANY_USER_ID)
    ...
    fun `Get DataException when getUserData is failure`() = runTest {
        ...
        whenever(userDataRepository.getUserData(ANY_USER_ID)).thenReturn(flowOf(resultFailure))
